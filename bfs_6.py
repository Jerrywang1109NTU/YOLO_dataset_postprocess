...
def trace_wire_bfs(image, start_point, center, die_rect, visited, threshold, start_r, die_dist_from_center):
    ...
    while q:
        ...
        for nx, ny in candidate_points:
            ...
            if len(path) <= 5 and image[ny, nx] > image[start_point[0], start_point[1]]: continue
        ...
        # --- 第一阶段排序 (按照原有逻辑) ---
        if len(path) < 5:
            valid_candidates.sort(key=lambda c: (c["dist_to_center"], c["line_sum"]))
        else:
            valid_candidates.sort(key=lambda c: (c["line_sum"]))
        
        # --- 新增：第二阶段排序 (对前2个结果按角度排序) ---
        # 1. 取出初步排序后的前2名候选点
        intermediate_candidates = valid_candidates[:2]
        
        # # 2. 仅对这2个点，按照角度偏差（越小越好）进行最终排序
        # if (len(path) >= 11 and len(path) <= 15) or len(path) >= 50:
        #     intermediate_candidates.sort(key=lambda c: (c["angle_diff"]))

        # 最终要探索的候选点就是这次排序的结果
        top_candidates = intermediate_candidates[:2]
