...
def trace_wire_bfs(image, start_point, second_anchor, center, die_rect, visited, threshold, start_r):
    ...
    search_sector_angle = 15.0 # 搜索扇形区的半角
    ...
    for _ in range(max_path_len):
        ...
        if len(path) == 1: # 初始步骤
            ...
            p_hist_far = path[-2] if len(path) >= 2 else path[0]
                ...
                if len(line_pixels) > 2:
                    for (lx, ly) in line_pixels[1:-1]:
                        if image[lx, ly] > image[py, px] + path_brightness_tolerance:
                            bright_points += 1
                        sumx += image[ly, ly]
                        max_p = max(max_p, image[lx, ly])
                    sumx /= len(line_pixels) 
        ...
        # 排序：优先选择颜色最深的，其次是离中心最近的
        if len(path) <= 5:
            valid_candidates.sort(key=lambda c: (c["gray_value"], c["bright_num"], c["angle_from_ref"]))
        else:
            valid_candidates.sort(key=lambda c: (c["gray_value"], c["angle_from_ref"], c["bright_num"]))
